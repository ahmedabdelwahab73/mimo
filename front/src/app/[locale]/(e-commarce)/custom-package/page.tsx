...
import { Loader2, Check, ShoppingBag, Send, PackageOpen } from 'lucide-react'
...
const CustomPackagePage = () => {
	...
	useEffect(() => {
		const fetchPackages = async () => {
			try {
				setLoading(true)
				// Fetch active custom packages without token (public API)
				const res = await apiFetch('/custom-packages', {
					headers: {
						'lang': locale // Only sending language as requested
						// Authorization header is intentionally omitted/ignored for this public view if using a modified fetch, but apiFetch handles it automatically if token absent. Back-end must have a public route.
					}
				})
				if (res.ok) {
					const data = await res.json()
					// Filter for active packages only on front-end just in case, though back-end should typically filter
					setPackages(data.filter((pkg: CustomPackage) => pkg.active))
				}
			} catch (error) {
		...
		<Container className='mt-30 mb-20'>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative items-start">
				<div className="lg:col-span-2 bg-[#c9beae87]   p-5 border-2 border-Brown-color rounded-[30px] grid grid-cols-1 gap-5">
					{packages.length > 0 ? packages.map((section) => (
									...
									return (
										<div
											...
											className={`cursor-pointer flex items-center justify-between gap-4 bg-background py-2
												 px-5 rounded-2xl        
														border border-border/50 hover:border-[#b0a090]/40 hover:shadow-lg 
										...
										>
											<div className='flex items-center gap-2'>
												...
												<h3 className={` 
												text-lg max-mxmdd:text-[14px] font-[500] text-foreground/80 leading-snug
				...
				<div className="lg:col-span-1 border border-border rounded-3xl p-6 bg-card sticky top-28 shadow-xl">
					...
					<button
						...
						className="w-full bg-[#03635a]
						 cursor-pointer disabled:cursor-not-allowed hover:opacity-90 disabled:from-muted disabled:to-muted disabled:opacity-50 text-white py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-3 transition-all shadow-lg shadow-emerald-500/20"
					>
						<Send size={20} />
						{t('FollowBooking')}
					</button>
